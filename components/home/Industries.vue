...
<template>
  <section id="industries" class="py-20 px-10 md:px-15  bg-gray-200 text-center">
    <div class="text-left flex flex-wrap lg:flex-nowrap gap-y-4 mb-10">
      <h3 class="lg:flex-1/2 text-4xl font-bold mb-2 uppercase">We Provide IT Services to Various Industries</h3>
      <p class="text-xl font-semibold lg:flex-1/2">We create solutions to meet the demands of modern industries, where
        reliability, integration, and performance are non-negotiable. Our track record spans sectors like healthcare,
        logistics, finance, and manufacturing, giving us the context to solve for your edge cases, not just the obvious
        ones.</p>
    </div>
    <!--  -->
    <div class="flex flex-col lg:flex-row gap-4">
      ...
      <div class="flex flex-col gap-4 w-full lg:w-1/3">
        <UButton v-for="service in services" :key="service.name" :icon="service.icon" :label="service.name"
          variant="outline" class="justify-start text-xl font-semibold rounded-xl px-6 py-4" :class="{
            'bg-primary text-white border-primary': selected.name === service.name,
            'text-gray-800 border-gray-300 hover:bg-gray-100': selected.name !== service.name
          }" @click="selectService(service)" />
      </div>

      <!-- Right content card -->
      <UCard class="w-full lg:w-2/3 lg:mt-0 mt-4 bg-gradient-to-bl from-black to-gray-400 text-white rounded-xl">
        ...
        <div class="grid grid-cols-1 gap-6" :class="{'md:grid-cols-2':selected.links.length != 0}">
          ...
          <div v-if="selected.links" class="flex flex-col gap-3">
            <ULink v-for="(link, index) in selected.links" :key="index" :to="link.url"
